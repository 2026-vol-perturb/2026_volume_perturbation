...
for gain in "${GAINS[@]}"; do
    ...
    sed "s/{{gain_db}}/${gain}/g" test_distant_config.template.yaml > "$gain_dir"/config.yaml || fail

    python "$WENET_DIR"/wenet/bin/recognize.py \
        --config "$gain_dir"/config.yaml \
        --test_data distant.list \
        --data_type raw \
        --checkpoint "$MODEL" \
        --result_dir "$gain_dir" \
        --gpu 0 \
